    ...
    constructor(props){
      ...
      verifyoutput = event => {
        ...
        if(this.state.startdate != '' && this.state.enddate!='') { 
            ...
            axios.post("http://localhost/wdm/building_manager_report.php", { user1:this.state.startdate , user2 : this.state.enddate})
            .then(res => {
              console.log(res);
      ...
      {this.state.dataSource.map((dataSource)=>{
        return(
          <tr>
            ...
            <th>{dataSource.bm_date}</th>
            <th>{dataSource.bm_contact}</th>
