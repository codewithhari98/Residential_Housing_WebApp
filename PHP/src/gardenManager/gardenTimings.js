...
class GardenTimings extends Component {
  state = {
    ...
    endtime: ''
     ...
     componentDidMount() {   
      axios.get("http://localhost/wdm/garden_timing_get.php")
          .then(res => {
    ...
    handleSubmit = event => {    
    axios.post("http://localhost/wdm/garden_timing_post.php", { user1:this.state.email,user2:this.state.starttime,user3:this.state.endtime
    }).then(res => {
      console.log("hello1")
          console.log(res);
          console.log(res.data);
