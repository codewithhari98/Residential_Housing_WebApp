...
class PoolTimings extends Component {
  state = {
    ...
    endtime: ''
     ...
     componentDidMount() {   
      axios.get("http://127.0.0.1:8000/getPoolTimingRoute")
          .then(res => {
    ...
    handleSubmit = event => {
    axios.put(`http://127.0.0.1:8000/updatePoolTimingRoute/${this.state.email}`, { user1: this.state.email, user2:this.state.starttime, user3:this.state.endtime
    }).then(res => {
      console.log("hello1")
          console.log(res);
    ...
    <thead>
      ...
      <td><button styles="font-size:1.2vw;" onClick={this.handleSubmit}>Update</button></td>
                            ...
                            <div className = "garden-grid-container">
                                ...
                                <div>
                                    <p> <b> People Subscribed to Pool</b></p>
                                            <Link to="/ReadPool">    
                                            <img border="0" alt="Sorry ! unavailable" src="images/pool.svg"/>
                                            </Link>
                                </div>
